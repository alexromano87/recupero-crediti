...
import { alertsApi, type Alert, type CreateAlertDto, type UpdateAlertDto } from '../api/alerts';
...
export function AlertsPage() {
  ...
  const handleSubmit = async () => {
    try {
      if (!formData.praticaId || !formData.titolo || !formData.dataScadenza) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-50">Alert & Scadenze</h1>
          <p className="max-w-xl text-xs text-slate-500 dark:text-slate-400">
            Gestisci alert e scadenze per le pratiche. Monitora le attività in scadenza.
          </p>
        </div>
        <button
            ...
            return (
              <div
                ...
                className={`rounded-xl border p-4 transition-all ${
                  !alert.attivo
                    ? 'opacity-50 border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50'
                    : alert.stato === 'chiuso'
