...
export function DocumentiPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      if (currentCartellaId) {
        const [docs, folders] = await Promise.all([
          documentiApi.getAllByCartella(currentCartellaId),
          cartelleApi.getAll(),
        ]);
        setDocumenti(docs);
        setCartelle(folders.filter((c) => c.cartellaParent?.id === currentCartellaId));

        // Build breadcrumb
        const ancestors = await cartelleApi.getAncestors(currentCartellaId);
  ...
  const handleMoveDocument = async () => {
    ...
    try {
      await documentiApi.update(documentToMove.id, {
        cartellaId: targetCartellaId || undefined,
      });
  ...
  const handleCreateFolder = async () => {
    ...
    try {
      ...
      alert('Errore durante l\'operazione sulla cartella');
    }
  };
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition shadow-lg shadow-indigo-500/30"
            >
        ...
        <div className="mb-4 flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          <div className="relative">
            ...
            <select
              ...
              onChange={(e) => setSelectedPraticaId(e.target.value || null)}
              className="pl-10 pr-8 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 appearance-none cursor-pointer"
            >
              <option value="">Tutte le pratiche</option>
              {pratiche.map((pratica) => (
                <option key={pratica.id} value={pratica.id}>
                  {pratica.cliente?.ragioneSociale} - {pratica.debitore?.ragioneSociale || pratica.debitore?.cognome}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredCartelle.map((cartella) => (
                <div
                  key={cartella.id}
                  className="group relative bg-white dark:bg-slate-800 rounded-lg border-2 border-slate-200 dark:border-slate-700 p-4 hover:shadow-lg transition"
                >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2 mt-3">
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition"
                        >
                        ...
                        <button
                          ...
                          className="px-2 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition"
                        >
                        ...
                        <button
                          ...
                          className="px-2 py-1.5 text-xs font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition"
                        >
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredDocumenti
                ...
                <div
                  key={doc.id}
                  className="group relative bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4 hover:shadow-lg transition"
                >
                  <div className="flex items-start gap-3">
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/40 transition"
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-semibold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition"
                  ...
                  <div className="mt-2 flex gap-2">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition"
                      title="Scarica"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/40 transition"
                      title="Elimina"
                    >
                      <Trash2 className="h-3.5 w-3.5" />
                    </button>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-lg w-full p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 dark:file:bg-indigo-900/50 dark:file:text-indigo-400"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setUploadPraticaId(e.target.value)}
                  className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                ...
                className="px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition"
              >
                Annulla
              </button>
              <button
                ...
                className="px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition"
              >
                Carica
              </button>
            </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    value={folderColore}
                    onChange={(e) => setFolderColore(e.target.value)}
                    className="flex-1 px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
              </div>
              ...
              {!editingFolder && (
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFolderPraticaId(e.target.value)}
                    className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  >
                    <option value="">Nessuna pratica</option>
                    {pratiche.map((pratica) => (
              ...
              {folderDocuments.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {folderDocuments.map((doc) => (
                    <div
                      key={doc.id}
                      className="bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 p-4 hover:shadow-md transition"
                    >
                      <div className="flex items-start gap-3">
                        {getFileIcon(doc.tipo)}
                        <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex gap-2">
                        <button
                          onClick={() => handleDownload(doc)}
                          className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 rounded hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 text-xs font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded hover:bg-red-100 dark:hover:bg-red-900/40 transition"
                        >
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setTargetCartellaId(e.target.value)}
                  className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
                  ...
                  {cartelle
                    ...
                    .map((cartella) => (
                      <option key={cartella.id} value={cartella.id}>
                        {cartella.nome}
                        {cartella.pratica && ` (${cartella.pratica.cliente?.ragioneSociale})`}
                      </option>
                    ))}
                </select>
              </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-auto p-6">
              ...
              {viewingDocument.pratica && (
                <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-800 mb-4">
              ...
              <div className="mt-6 p-8 bg-slate-100 dark:bg-slate-900 rounded-lg border-2 border-dashed border-slate-300 dark:border-slate-700 text-center">
